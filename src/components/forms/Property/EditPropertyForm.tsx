...
interface EditPropertyFormProps {
  ...
  onFormDataChange: (data: EditPropertyFormData) => void;
  onSubmit: (e: React.FormEvent) => void;
  isSubmitting: boolean;
  errors?: Partial<Record<keyof EditPropertyFormData, string>>;
}
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-6">
      <ImageUploader
        ...
        initialImageUrls={Array.isArray(formData.property_images) ? formData.property_images.filter(img => typeof img === 'string') : []}
      />
      <div className="bg-white rounded-xl shadow-sm p-4">
        <h3 className="text-lg font-semibold mb-4">Edit Property Information</h3>
        <Input
          label="Title"
      ...
      <SelectAmeneties
        ...
        onChange={(newAmenities) => {
          // Update amenities in form data and trigger parent state update
          onFormDataChange({ ...formData, amenities: Array.isArray(newAmenities) ? [...newAmenities] : [] });
        }}
      />
